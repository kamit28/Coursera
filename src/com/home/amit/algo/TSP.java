...
public class TSP {
	...
	public static void main(String[] args) throws FileNotFoundException {
		String inputFileName = "/Users/Anshu/Downloads/tsp.txt";
		Scanner in = new Scanner(new File(inputFileName));
		number_of_cities = in.nextInt();
		double[][] xy_points = new double[number_of_cities + 1][2];
		cities = new ArrayList<Integer>();
		for (int i = 1; i < number_of_cities + 1; i++) {
			xy_points[i][0] = in.nextDouble();
			xy_points[i][1] = in.nextDouble();
			cities.add(i);
		}
		in.close();
		...
		for (int i = 0; i < myPowerSetPrevious.size(); i++) {
			if (myPowerSetPrevious.get(i).size() == 1
					& myPowerSetPrevious.get(i).contains(1)) {
				A[i][1] = 0;
			} else
		...
		while (count < sizeTogether) {// Set<Integer> s : myPowerSetNext){ //
										// for each subset S

			Set<Integer> s = myPowerSetNext.get(count
					- myPowerSetPrevious.size());
			if (s.size() >= 2 & s.contains(1)) {
				...
				for (Integer j : s) { // for each j vertex
					if (j != 1) {
						...
						for (Integer k : s) {
							if (k != j) {
								double dist = A[index][k]
										+ distance(xy_points[k][0],
												xy_points[k][1],
												xy_points[j][0],
												xy_points[j][1]);
								if (minDist == -1) {
		...
		for (int j = 2; j <= number_of_cities; j++) {
			double dist = A[A.length - 1][j]
					+ distance(xy_points[j][0], xy_points[j][1],
							xy_points[1][0], xy_points[1][1]);
			if (minDist == -1)
				minDist = dist;
			else if (minDist > dist) {
				minDist = dist;
