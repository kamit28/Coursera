...
public class Heap<E, K> {
	...
	/**
	 * Use {@link #naturalMin()}, {@link #naturalMax()} or
	 * {@link #custom(Comparator)} for creating a heap
	 * 
	 * @param comparator
	...
	/**
	 * Creates a heap with natural order of elements (i.e. heap which supports
	...
	/**
	 * Creates a heap with reversed natural order of elements (i.e. heap which
	 * supports "extractMax" operation)
	 */
	...
	/**
	 * Retrieves the stored value from the front, removes the front from the
	 * heap
	 * 
	...
	private void ensureNotEmpty() {
		if (heap.isEmpty()) {
			throw new IllegalStateException(
					"the heap is empty, cannot extract min");
		}
	}
	...
	/**
	 * Depending on the new value of key, it either calls
	 * {@link #decreaseKey(HeapNode, Object)} or
	...
	/**
	 * Holds the data that a heap node contains and plus some extra meta
	 * information like index and key.
	 * 
	 * @author Grigorev Alexey
	 */
	// Implemented as an inner class to expose some private methods only to the
	// heap
	public static class HeapNode<E, K> {
		...
		@Override
		public String toString() {
			return "HeapNode [value=" + value + ", key=" + key + ", index="
					+ index + "]";
		}
	}
}
